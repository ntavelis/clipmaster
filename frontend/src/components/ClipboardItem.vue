...
<template>
  <div
    ...
    :class="[selected ? 'bg-color8/50' : '', clipboard.keyboardActive ? '' : 'hover:bg-color8/50']"
    @click="handleCopy"
    @mouseenter="onMouseEnter"
    @mouseleave="onMouseLeave"
  >
    <span class="shrink-0 w-3 text-[10px] leading-4 text-color8 text-center">{{ index < 9 ? index + 1 : '·' }}</span>
    ...
    <button
      ...
      class="shrink-0 mt-0.5 text-color7 hover:text-accent cursor-pointer transition-transform"
    ...
    <div ref="iconRef" class="shrink-0 mt-0.5">
      <svg
        ...
        stroke-linecap="round"
        stroke-linejoin="round"
        class="w-4 h-4 text-color7 group-hover:text-accent transition-colors"
      >
