...
- Each instance starts an HTTPS server on `0.0.0.0` and a random OS-assigned port by default; both the bind IP and port are configurable
...
| `OMACLIP_SYNC_SERVER_IP` | `0.0.0.0` | IP address the clipboard sync HTTPS server listens on |
