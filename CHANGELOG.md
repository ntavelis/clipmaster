...
- Replaced `grandcat/zeroconf` with `hashicorp/mdns`; registration and discovery now run per eligible IPv4 interface and aggregate each browse cycle before expiring peers.
...
- Pin icon is no longer shown on remote clipboard items (pinning is local-only)
- Pinned items now display a 2px accent-coloured left border for clear visual distinction
